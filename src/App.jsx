        ...
        {mode === 'select' ? (
          ...
          <div className="space-y-4">
            ...
            {mode === 'lernend' && (
              <>
                <Input 
                  label="Zugangscode von Berufsbildner/in" 
                ...
                <p className="text-stone-500 text-sm text-center">
                  Du erhältst den Code von deiner/m Berufsbildner/in
                </p>
              </>
            )}
...
const AdminBereich = ({ berufsbildner, lernende, rapporte, onLogout, onRefresh }) => {
  ...
  const createBerufsbildner = async () => {
    ...
    try {
      ...
      setSuccess(`✅ Berufsbildner/in "${newBBName}" erstellt! Login mit ${newBBEmail} möglich.`);
  ...
  const deleteBerufsbildner = async (id) => {
    if (lernende.some(l => l.berufsbildnerId === id)) { alert('Hat noch Lernende zugewiesen!'); return; }
    try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900">
      <nav className="bg-stone-800/80 backdrop-blur-md border-b border-stone-700 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🏗️</span>
            <span className="font-semibold text-stone-100">MauerwerkCheck</span>
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        ...
        <Card>
          ...
          <Button variant="primary" className="mt-4" onClick={createBerufsbildner} disabled={!newBBName || !newBBEmail || saving}>
            {saving ? 'Erstellen...' : 'Berufsbildner/in erstellen'}
          </Button>
          <p className="text-stone-500 text-sm mt-2">
            Berufsbildner/in erhält einen Anmelde-Link per E-Mail (kein Passwort nötig)
          </p>
        </Card>
        <Card>
